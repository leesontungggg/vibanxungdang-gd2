$(function () {
    ...
    showErrorWithMessage = function (type, message) {
        switch(type) {
            case registrationType.REGISTER:
                ...
                TweenMax.to(registerError, 0.3, {alpha: 1, ease: Sine.easeOut});
    ...
    hideError = function (type) {
        switch(type) {
            case registrationType.REGISTER:
                TweenMax.killTweensOf(registerError)
                TweenMax.to(registerError, 0.3, {alpha: 0, ease: Sine.easeOut});
            break;
            case registrationType.LOGIN:
                TweenMax.killTweensOf(loginError)
    ...
    onLoginClick = function () {
        ...
        const loginData = {phone: phone , refcode: null};
        if (phone.length > 0) {
            if (isLocal) {
                if (refcode.length > 0) {
